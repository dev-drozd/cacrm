var _d = document, makeStat = {
	...
	init: function(o) {
		...
		// svg create
		var svgEl = _d.createElementNS(makeStat.xmlns, 'svg');
		...
		var svgCtnr = _d.getElementById(o.id);
    	svgCtnr.appendChild(svgEl);

		// plot area		
		makeStat.g = _d.createElementNS(makeStat.xmlns, 'g');
	    svgEl.appendChild(makeStat.g);
		makeStat.g.setAttribute('transform', 'matrix(1,0,0,-1,' + s / 2 + ',' + bHeight + ')');

		// oxy area		
	    var xy = _d.createElementNS(makeStat.xmlns, 'g');
		...
		// grid area
		var grid = _d.createElementNS(makeStat.xmlns, 'g');
		...
		// lines area
		var lines = _d.createElementNS(makeStat.xmlns, 'g');
		makeStat.g.appendChild(lines);

		// points area
		var points = _d.createElementNS(makeStat.xmlns, 'g');
		makeStat.g.appendChild(points);
		
		// tips area
		var tips = _d.createElementNS(makeStat.xmlns, 'g');
		...
		if (o.data[0]['points']) {
			...
			o.data.forEach(function(v, j) {
				for (var i = 0; i < o.data[j]['points'].length; i++) {
					...
					tTip = _d.createElementNS(makeStat.xmlns, 'g');
		...
		var line = _d.createElementNS(makeStat.xmlns, 'line');
		...
		var polygon = _d.createElementNS(makeStat.xmlns, 'polygon');
	    polygon.setAttribute('points', o.points);
	    polygon.setAttribute('fill', o.fill);
	    g.appendChild(polygon);
	}, cCircle: function(o, g) {
		var circle = _d.createElementNS(makeStat.xmlns, 'circle');
		...
		var rect = _d.createElementNS(makeStat.xmlns, 'rect');
		...
		var text = _d.createElementNS(makeStat.xmlns, 'text');
		...
		var textNode, tspan;
		if (o.text) {
			textNode = _d.createTextNode(o.text);
			tspan = _d.createElementNS(makeStat.xmlns, 'tspan');
			tspan.appendChild(textNode);
			text.appendChild(tspan);
		}
		textNode = _d.createTextNode(o.tx + (o.px + (o.plusx || 0)).toFixed(0) + '; ' + o.ty + o.py.toFixed(0) + ';');
		tspan = _d.createElementNS(makeStat.xmlns, 'tspan');
		...
		var textNode = _d.createTextNode(o.text);
		text.appendChild(textNode);
		g.appendChild(text);
		return text;
	}, showTips: function(id, oid, did) {
		_d.querySelector('#' + oid + (did ? '_' + did + '_' : '') + '_tTip_' + id).style.opacity = 1;
		_d.querySelector('#' + oid + (did ? '_' + did + '_' : '') + '_tTip_' + id).style.visibility = 'visible';
	}, hideTips: function(id, oid, did) {
		_d.querySelector('#' + oid + (did ? '_' + did + '_' : '') + '_tTip_' + id).style.opacity = 0;
		_d.querySelector('#' + oid + (did ? '_' + did + '_' : '') + '_tTip_' + id).style.visibility = 'hidden';
	}
};