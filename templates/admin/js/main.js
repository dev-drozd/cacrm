...
function fnGet(e){
	var l = e.list;
	if(!l) return;
	var r = $(l).find('option[value="'+e.value+'"]');
	if(r.length){
		e.value= "";
		Page.get(r.attr('data-href'));
	}
}
...
var Doload = function(e) {
	...
	$.post(($(e).attr('action') || location.pathname), {
		...
		current_status: $('[name="current_status"]').val(),
		instore: $('input[name="instore"]').val(),
		all: $('input[name="all"]').val(),
		updated: $('input[name="updated"]').val(),
		pickedup: $('input[name="pickedup"]').val(),
...
var quote = {
	...
	del: function(a){
		$.post('/quote/del', {id: a}, function(r){});
		$('#quote_'+a).remove();
	}
};
...
function addField(a,b){
	mdl.close();
	box.open({
		title: 'Adittion field',
		content: '\
		<div class="iGroup act">\
			<label>Type:</label>\
			<select name="type">\
		...
		<div class="iGroup">\
			<label>Price:</label>\
			<input type="number" name="price" min="0" step="0.001" required>\
		</div>\
		...
		submit: ['<span class="fa fa-save"></span> Add', function(a){
			loadBtn.start($(this.button));
			$.post('/issues/addfield', {
				id: this.id.value,
				...
				if(r == 'OK'){
					...
					alr.show({
						class: 'alrDanger',
						content: 'Something went wrong, contact the developer',
						delay: 2
					});
				}
			});
...
var user = {
	...
	webPhoto: function() {
		...
		mdl.open({
			id: 'newUser',
			title: 'New User',
			content: '<form class="uForm" method="post" onsubmit="user.addShort(this, event, '+a+');">\
...
var objects = {
	close: function(a,b){
		if(confirm('Are you sure you want to close this store?')){
			if(b){
				$('#object_'+a).addClass('close');
				$('.clstore').attr('href', 'javascript:objects.close('+a+')').html('<span class="fa fa-play"></span> Open object');
			} else {
				$('#object_'+a).removeClass('close');
				$('.clstore').attr('href', 'javascript:objects.close('+a+', 1)').html('<span class="fa fa-stop"></span> Close object');
			}
			$.post('/objects/close', {
				id: a,
				close: b
...
// Inventory
var inventory = {
	delAddField: function(a){
		var  p = $(a).parents('.tr'),
			id = p[0].id.replace('addition_field_', '');
		p.remove();
		$.post('/inventory/del_addition_fields', {
			id: Number(id)
		})
	},
	approveField: function(a){
		$('#addition_field_'+a).remove();
		$.post('/inventory/send_approve_field', {id: a});
	},
	...
	addModel: function(id, n, p) {
		...
		if(typeof id == 'string'){
			step = id;
			var brand = $('div[name=brand]').data();
			if(brand.value.length > 0){
				var p = {
					[brand.value]: {
						name: $('div[name=brand] input').val()
					}
				};
			}
			id = undefined;
        ...
        $.post('/inventory/allCategories', {
            ...
            if (r) {
                ...
                mdl.open({
                    ...
                    content: '<form  method="post" onsubmit="inventory.sendModel(this, event, ' + (id || 0) + ', \''+step+'\');">\
						<div class="iGroup">\
							<label>' + lang[20] + '</label>\
							<input type="text" name="name" value="' + (n || '') + '">\
						</div>\
    ...
    sendModel: function(f, event, id, eid) {
        event.preventDefault();
        loadBtn.start($(event.target).find('button[type="submit"]'));
        var data = new FormData(),
            form = $(f),
        ...
        if (!form.find('input[name="name"]').val().length) {
            ...
            $.ajax({
                ...
                if (r > 0) {
					if(eid){
						$(eid).data().value = r;
						$(eid).find('input').val(data.get('name'));
					} else {
						alr.show({
							class: 'alrSuccess',
							content: lang[8],
    ...
    sendCategory: function(f, event, id, eid) {
        event.preventDefault();
        loadBtn.start($(event.target).find('button[type="submit"]'));
        var data = new FormData(),
            form = $(f),
				...
				/* $.post('/inventory/allTypes', {
					...
					if (r) {
						...
						mdl.open({
							...
							content: '<form  method="post" onsubmit="inventory.sendCategory(this, event, ' + (id || 0) + ', \''+step+'\');">\
								<div class="iGroup">\
									<label>' + lang[20] + '</label>\
									<input type="text" name="name" value="' + (n || '') + '">\
								</div>\
        ...
        $.post('/inventory/allCategories', {
            ...
            if (r) {
                ...
                mdl.open({
                    ...
                    content: '<form  method="post" onsubmit="inventory.sendType(this, event, ' + id + ', \''+step+'\');">\
						<div class="iGroup">\
							<label>' + lang[20] + '</label>\
							<input type="text" name="name" value="' + (n || '') + '">\
						</div>\
    ...
    sendType: function(f, event, id, eid) {
        event.preventDefault();
        loadBtn.start($(event.target).find('button[type="submit"]'));
        var data = {
                id: id,
    ...
    addInv: function(f, event, id, n) {
        ...
        if ((!form.find('input[name="category"]').length && !form.find('input[name="category"]').next().find('input').val()) && !form.find('input[name="name"]').length) {
            ...
            if ($(f).find('select[name="type_id"]').val() == 0) {
                ...
                data.append('publish', $(f).find('input[name="publish"]').val());
				...
				data.append('pathname', $(f).find('input[name="pathname"]').val() || '');
				data.append('stitle', $(f).find('input[name="title"]').val() || '');
				data.append('description', $(f).find('textarea[name="description"]').val() || '');
				data.append('keywords', $(f).find('input[name="keywords"]').val() || '');
				data.append('canonical', $(f).find('input[name="canonical"]').val() || '');
        ...
        if (err) {
            ...
            $.ajax({
                ...
                if (!r) {
					...
					else if(location.pathname.indexOf('stock') >= 0){
						Page.get('/store');
					} else {
						if ($_GET('backusr'))
		...
		$.post('/settings/allForms', {}, function(r) {
			...
			mdl.open({
				...
				content: $('<div/>', {
					...
					id: 'parent_ids',
					name: 'parent_ids',
					json: '/inventory/allNewStatuses?curent_id='+id+'&warranty='+warranty,
					res: 'list',
					search: 'ajax10',
					multiple: true
				}).append())).append(warranty == 0 ? $('<div/>', {
					class: 'iGroup',
					html: $('<label/>', {
						html: '% from total'
					})
    ...
    sendStatus: function(el, id, warranty) {
        ...
        if (!f.find('input[name="name"]').val().length) {
			...
			data.parent_ids = f.find('div[name="parent_ids"]').data('value').join(',');
            data.pointGroup = {};
...
var purchases = {
	...
	addShipment: function(a){
		$('input[name="total"]').val((Number($('input[name="price"]').val())*Number($('input[name="quantity"]').val()))+Number(a));
	},
	...
	changePrice: function() {
		...
		$('input[name="total"]').val(((price * parseFloat($('input[name="quantity"]').val()))+parseFloat($('input[name="shipment_cost"]').val())).toFixed(2));
	...
	getLink: function(el) {
        ...
        $.get('/parser?lnk=' + link, function(r) {
            switch (r) {
                ...
                default:
					var p = Number(r.price.replace(/[^0-9.]/g, '')).toFixed(2),
						sp = Number(r.sale_price.replace(/[^0-9.]/g, '')).toFixed(2);
					$(el).parents('.uForm').find('input[name="name"]').val(r.title.trim());
					$(el).parents('.uForm').find('input[name="price"]').val(p);
					$(el).parents('.uForm').find('input[name="sale"]').val(sp).attr('min', sp);
					$(el).parents('.uForm').find('input[name="quantity"]').val(1);
					$(el).parents('.uForm').find('input[name="total"]').val(p);
			...
			$('input[name=shipment_cost]').trigger('input');
    ...
    send: function(f, event, id) {
		...
		if(!$('input[name="shipment_cost"]').val()){
            alr.show({
                class: 'alrDanger',
                content: 'Please insert shipment cost',
                delay: 2
            });
			return false;
		}
...
var issues = {
	...
	addRandomFeedback: function(id, type) {
		var rating = parseInt($('#feedback_' + id).attr('data-stars')),
			star = '';
		for(j = 1; j <= 5; j++){
		...
		mdl.open({
			...
			content: $('<div/>', {
				...
				name: 'feed_comment',
				html: $('#comment_' + id).text().trim()
			}))).append($('<div/>', {
				class: 'sGroup',
				html: $('<button/>', {
					type: 'button',
					class: 'btn btnSubmit',
					onclick: 'issues.sendRandomFeedback(event, '+id+', \''+type+'\')',
					html: 'Send'
				})
			}))
		});
	...
	sendRandomFeedback: function(event, id, type){
		event.preventDefault();
		...
		$.post('/issues/is_warranty', {
			...
			mdl.open({
				...
				cb: function() {
					...
					if ($('.devStatus').attr('data-id') != 0) st[$('.devStatus').attr('data-id')] = {name: $('.devStatus > span').text().replace('Status: ', '')};
...
/* 		else
	...
	openDualWindow: function(a){
		setTimeout(function(){
			window.open('/pdf/barcode/'+a);
		},1);
		setTimeout(function(){
			to_print('/forms?type=issue&id=5&issue_id='+a, 'issue №'+a);
		}, 500);
	},
    del: function(id) {
        $.post('/issues/del', {
	...
	newPur: function(el) {
		...
		$.get('/parser?lnk=' + el.value, function(r) {
            switch (r) {
                ...
                default:
					var title = r.title ? r.title.trim() : '', 
						img = r.image || '', 
						price = r.price || '', 
						pprice = r.price ? parseFloat(r.price.replace(/(\D*)/i, '')) : '', 
						sale_price = parseFloat(r.sale_price), 
						itemid = r.code ? r.code.trim() : '';

				$(el).parent().next().append('<div class="pnThumb">'+(
						img ? '<img src="'+img+'">' : ''
				)+'</div>\
					<div class="pNewText">\
						<div class="pnName">'+title+'</div>\
					</div>\
					<div class="itemId">'+itemid+'</div>\
					<div class="pnPrice">'+price+'</div>\
					<input type="number" class="npPrice" name="p_price" step="0.001" min="'+pprice+'" value="'+pprice+'" placeholder="Price (Our Cost)"'+(
						pprice > 0 ? 'hidden' : ' '
					)+'>\
					<input type="number" class="npPrice" name="price" step="0.001" min="'+sale_price+'" value="'+Math.ceil(sale_price)+'" placeholder="Sale price" onchange="purchases.minPrice(this)">\
					<input type="number" class="npPrice" step="0.001" min="0" name="shipment_cost" placeholder="Shipment cost">\
					<input type="text" class="npPrice" name="salename" placeholder="Sale name">'
				);
            }
        }, 'json');
	},
		...
		$.post('/inventory/all', {
			...
			if (r) {
				var items = '',
					lId = 0;
				$.each(r.list, function(i, v) {
					items += '<li data-value="' + v.id + '" data-price="'+ currency_val[v.currency || 'USD'].symbol + parseFloat(v.price).toFixed(2) + '" data-cost-price="'+ currency_val[v.currency || 'USD'].symbol + parseFloat(v.cost_price).toFixed(2) + '" data-currency="' + (v.currency || 'USD') + '"' + ($('input[name="inventory"]').length ? ' data-object="' + v.object + '"' : '') + '>' + v.name + '</li>';
					lId = v.id;
				});
			...
			$.post('/inventory/all', {
				...
				if (r) {
					var items = '',
						lId = 0;
					$.each(r.list, function(i, v) {
						items += '<li data-value="' + v.id + '" data-price="'+ currency_val[v.currency || 'USD'].symbol + parseFloat(v.price).toFixed(2) + '" data-cost-price="'+ currency_val[v.currency || 'USD'].symbol + parseFloat(v.cost_price).toFixed(2) + '" data-currency="' + (v.currency || 'USD') + '" data-object="' + v.object + '">' + v.name + '</li>';
						lId = v.id;
					});
					$('#inventory > ul').html(items).sForm({
						action: '/inventory/all',
    ...
    sendInv: function(f, id, type, cid) {
        ...
        if (!form.find('input[name="' + name + '"]').data())
			...
			if ($('input[name="new_purchase"]').val()) {
				...
				data.append('itemID', $('.itemId').text());
				data.append('cprice', $('input[name="price"]').val() || 0);
				data.append('p_price', $('input[name="p_price"]').val() || 0);
				data.append('shipment_cost', $('input[name="shipment_cost"]').val() || 0);
				data.append('link', $('input[name="new_purchase"]').val());
				data.append('salename', $('input[name="salename"]').val());
				if ($('input[name="p_price"]').val() == 0)
					err = 'Price (Our Cost) can not be 0';
				if ($('input[name="price"]').val() == 0)
					err = 'Price can not be 0';
				if (!$('input[name="shipment_cost"]').val())
					err = 'Please insert shipment cost';
				if (!$('input[name="salename"]').val())
					err = 'Sale name can not be empty';
			}
        }
    ...
    pPrice: function(el, id, iss, type) {
        ...
        $(el).attr('ondblclick', '').html($('<input/>', {
            ...
            step: '0.01',
            min: '0.00',
			style: 'position: absolute;margin-left: -15px;width:100px;',
			...
			onkeypress: 'if(event.keyCode == 13) issues.sendPPrice(this, \'' + id + '\', ' + iss + ', \'' + type + '\');',
    ...
    sendPPrice: function(el, id, iss, type) {
        ...
        if (err) {
			...
			if ($('input[name="pPrice"]').val() == $('input[name="pPrice"]').attr('old')) {
				var p = ((parseFloat($('input[name="pPrice"]').val()) + parseFloat($('input[name="pPrice"]').attr('inv_price'))) * Number($('input[name="pPrice"]').attr('quantity') || 1)).toFixed(2);
				$(el).parent().attr('data-price', p).text(p).attr('ondblclick', 'issues.pPrice(this, \'' + id + '\', ' + iss  + ', \'' + type + '\')');
				//$(el).parent().text(((parseFloat($('input[name="pPrice"]').val()) + parseFloat($('input[name="pPrice"]').attr('inv_price'))) * Number($('input[name="pPrice"]').attr('quantity') || 1)).toFixed(2)).attr('ondblclick', 'issues.pPrice(this, \'' + id + '\', ' + iss  + ', \'' + type + '\')');
			} else {
				$.post('/issues/send_price', data, function(r) {
					...
					if (!r) {
						...
						if (type == 'service_info') {
							...
							$('#service_' + id).find('.servPrice').attr('data-price', $('input[name="pPrice"]').val()).text($('input[name="pPrice"]').val()).attr('ondblclick', 'issues.pPrice(this, \'' + id + '\', ' + iss  + ', \'' + type + '\')');
						} else {
							var p = parseFloat($('input[name="pPrice"]').val()).toFixed(2);
							$(el).parent().attr('data-price', p).text(p).attr('ondblclick', 'issues.pPrice(this, \'' + id + '\', ' + iss  + ', \'' + type + '\')');
						}
						total.fTotal();
					}
	...
	report: function(el, main) {
		...
		if (err) {
			...
			if (!main) {
				data.store_ids = Object.keys($('input[name="object"]').data()).join(',');
				data.date_start = $('#calendar > input[name="date"]').val();
				data.date_finish = $('#calendar > input[name="fDate"]').val();
			}
			
			$.post('/issues/report_result', data, function(r) {
				loadBtn.stop($(el));
				if (r) {
					$('#report').html(r.report).append($('<div/>', {
						class: 'roTotal',
						html: 'Total jobs: ' + r.total
					}));
					if ($('#report').next().hasClass('profile-loader'))
						$('#report').next().remove();
				} else {
	...
	reportItem: function(a,b,c,d){
		$.post('/issues/report_item_result', {
			store_id: a,
			date_start: b,
			date_finish: c
		}, function(a){
			$(d).find('p').html(a.content);
...
var invoices = {
    ...
    addInventory: function() {
        mdl.open({
            ...
            content: '<div class="form">\
				<div class="tabs">\
					<div class="tab" data-title="'+lang[68]+'" id="inv">\
						<div id="inventory" class="iGroup price quantity">\
							<ul></ul>\
						</div>\
					</div>\
					<div class="tab" data-title="'+lang[67]+'" id="serv">\
						<div id="services" class="iGroup price">\
							<ul></ul>\
						</div>\
					</div>\
					<div class="tab" data-title="Order parts" id="purch">\
						<div id="purchases" class="iGroup">\
							<ul></ul>\
						</div>\
						<div class="iGroup fw">\
							<label>New purchase</label>\
							<input type="text" name="new_purchase" oninput="issues.newPur(this)">\
						</div>\
						<div></div>\
					</div>\
					<div class="tab" data-title="Buy from customer" id="buy_inventory">\
						<div id="tradein" class="iGroup">\
							<ul></ul>\
						</div>\
						<div class="newTradeins" id="newTradein">\
							<table class="tradein_table">\
								<thead>\
									<tr>\
										<th>Name</th>\
					...
					<div class="tab" data-title="Addition field" id="addition">\
						<div class="iGroup">\
						  <label>Type:</label>\
				...
				<div class="sGroup">\
					<button type="button" class="btn btnSubmit" onclick="invoices.chooseInv();">'+lang[65]+'</button>\
				</div>\
			</div>',
            cb: function() {
                $('#page').tabs();
				$('select[name=addition_type]').select();
				$('#addition > input[name=tax]').checkbox();
                $.post('/inventory/all', {
                    noCust: 1,
                    type: 'stock',
	...
	selectAll: function() {
		 if (!Object.keys($('input[name="inventory"]').data()).join(',') && !Object.keys($('input[name="tradein"]').data()).join(',') && !Object.keys($('input[name="services"]').data()).join(',') && !Object.keys($('input[name="invoices"]').data()).join(',') && !Object.keys($('input[name="purchases"]').data()).join(',') && !$('#addition input[name=name]').val()) {
            alr.show({
				class: 'alrDanger',
            ...
            var inv = $('input[name="inventory"]').data(),
				...
				adtn = $('#addition'),
				aname = adtn.find('input[name=name]').val(),
				aprice = adtn.find('input[name=price]').val(),
				atype = adtn.find('select[name=addition_type]').val(),
				tax = adtn.find('input[name=tax]').val(),
				adata = {};
				
				if(aname && aprice && atype){
					$.post('/inventory/send_addition', {
						in_id: location.pathname.replace('/invoices/edit/',''),
						type: atype,
						name: aname,
						price: aprice,
						store_id: Object.keys($('input[name=object]').data()).join(','),
						tax: tax
					});
				}
			...
			if(Object.keys(adata).length > 0 && Object.keys(adata).length < 2){
				alr.show({
					class: 'alrDanger',
					content: 'Not all additional fields are filled out!',
					delay: 2
				});
				return false;
			}
			...
			if(Object.keys(adata).length >= 2){
				adata = {[(new Date()).getTime()]: adata};
				$('input[name="addition"]').data(adata);
				invoices.eachService(adata, 'addition');
			}
			...
			$.each(el, function (i, v) {
				if (!$('.mInvoice #tr_' + type + '_' + i).length) {
					$('.tbl.payInfo.mInvoice').append($('<div/>', {
						...
						class: 'td w10',
						html: ['stock', 'service', 'addition'].indexOf(type) >= 0 ? $('<input/>', {
							type: 'number',
							name: 'quantity',
							class: 'quickEdit',
							max: type == 'addition' ? '' : v.quantity,
							min: '1',
							value: '1',
						...
						class: 'td w10',
						html: (type == 'tradein' ? 'no' : (
							type == 'addition' ? (v.tax ? 'yes' : 'no') : 'yes'
							)
						)
					})))
				}
	...
	sendMorePrice: function(el, i, type) {
		if (parseFloat($('input[name="morePrice_' + i +'"]').val()) <= parseFloat($('input[name="morePrice_' + i +'"]').attr('oldvalue'))) {
			...
			var inv = $('input[name="' + (type == 'stock' ? 'inventory' : (type == 'addition' ? type : (type + 's'))) + '"]').data();
			inv[i].price = $(el).val();
			$('input[name="' + (type == 'stock' ? 'inventory' : (type == 'addition' ? type : (type + 's'))) + '"]').data(inv);
		}
	},
    delInvItem: function(el, type, id) {
        type = (type == 'stock') ? 'inventory' : ((type == 'tradein' || type == 'addition') ? type : type + 's');
        $(el).parent().parent().remove();
    ...
    total: function(el) {
        var subtotal = 0,
			...
			tradein = 0,
            pSub = 0,
            tax = $('input[name=tax_exempt]').val() ? 0 : $('input[name="object"]').data()[Object.keys($('input[name="object"]').data())].tax,
            discount = 1;
	...
	onExempt: function(e){
		$(e).hide();
		$('.tax_exempt').find('input').show().focus();
	},
    create: function(el, id, estimate) {
        ...
        if (!estimate && ($('input[name="customer"]').length ? !Object.keys($('input[name="customer"]').data()).join(',').length : 0) && parseFloat($('#total').text()) > quick_sell) {
            ...
            err = lang[63];
        } else if (!Object.keys($('input[name="addition"]').data()).join(',').length && !Object.keys($('input[name="services"]').data()).join(',').length && !Object.keys($('input[name="inventory"]').data()).join(',').length && !Object.keys($('input[name="tradein"]').data()).join(',').length && !Object.keys($('input[name="purchases"]').data()).join(',').length && !$('[data-type="onsite"]').length && !$('div[data-currency="USD"]').length) {
            err = lang[62];
        } else {
            ...
            data.additions = JSON.stringify($('input[name="addition"]').data());
			...
			data.tax_exempt = $('input[name=tax_exempt]').val();
    ...
    issCreate: function(id, c, o, d) {
	...
	}, makePayment: function(el, id, p, g) {
		loadBtn.start($(el));
			var data = {};
			if (($('select[name="method"]').val() != 'cash' || p) && Math.abs(parseFloat($('input[name="cash"]').val())) > Math.abs(parseFloat($('#due').text()))) {
				...
				data.service_charge = $('#charge').attr('data-percent');
				if(g)
					data.iss_complete = 1;
		...
		if ($('select[name="method"]').val() == 0) {
			...
			} else if($('.uForm .td a[href^="/issues/view/"]').length){
				mdl.open({
					id: 'appliedPartialJob',
					title: 'The question',
					content: $('<div/>', {
						class: 'aCenter',
						html: $('.uName > div > a').text()+' has picked up his Device?'
					}).append($('<div/>', {
						class: 'cashGroup',
						html: $('<button/>', {
							type: 'button',
							class: 'btn btnSubmit ac',
							onclick: 'invoices.makePayment(this, ' + id + ', ' + p + ',1);',
							html: 'Yes'
						})
					}).append($('<button/>', {
						type: 'button',
						class: 'btn btnSubmit dc',
						onclick: 'invoices.makePayment(this, ' + id + ', ' + p + ');mdl.close();',
						html: 'No'
					})))
				})
			} else
...
var bugs = {
	videoShow: function(a){
		var src = a.getAttribute('video-src'),
			video = document.createElement('video');
			video.addEventListener("webkitfullscreenchange",function(){
				if(!document.webkitIsFullScreen) $(video).remove();
			}, false);
			$('body').append(video);
    ...
    add: function(id, imp) {
            ...
            content: $('<div/>', {
                ...
                html: $('<div/>', {
					...
					html: content || ''
                        ...
                        }).append('Click or drag and drop videos or images here')).append($('<div/>', {
                            class: 'thumbnails',
							html: images || ''
                        }))).append('We recommend to install the extension for Google Chrome <br /> <a href="https://chrome.google.com/webstore/detail/screencastify-screen-vide/mmeijimgabbpbgpdklnllpncmdofkcpn/related" target="_blank">Screen recording</a>').append($('<div/>', {
                    class: 'sGroup',
                    html: $('<button/>', {
                        class: 'btn btnSubmit', 
                        html: lang[18],
		...
		$('.dragndrop').upload({
			count: 0,
			accept: 'image/*,video/*',
			types: ['jpeg', 'jpg', 'png', 'gif', 'mp4', 'webm'],
			size: 200,
			multiple: true,
			max: 5,
			check: function(e){
				...
				if(!e.error){
					...
					switch(e.file.type){
						case 'video/mp4':
						case 'video/webm':
							img.src= '/templates/admin/img/videothumb.jpg';
							img.setAttribute('onclick', 'bugs.videoShow(this);');
							img.setAttribute('video-src', URL.createObjectURL(e.file));
						break;
						default:
        ...
        loadBtn.start($(el));
		$('#bugAdd .bugForm').hide();
		$('#bugAdd .mdlBody').append('<img src="/templates/admin/img/sb-loading.gif">');
                ...
                if (r) {
					...
					if (r.images) {
						$.each(r.images, function(i, v) {
							new_images += '<div class="thumb">'+(
								(v.split('.').pop() == 'mp4' || v.split('.').pop() == 'webm') ? '<img src="/templates/admin/img/videothumb.jpg" video-src="/uploads/videos/bugs/' + r.id + '/' + v + '" onclick="bugs.videoShow(this);">' : '<img src="/uploads/images/bugs/' + r.id + '/thumb_' + v + '" onclick="showPhoto(this.src);">'
							)+'</div>'
						})
					}
                    if (id) {
...
var cash = {
	...
	openCash: function(t, a) {
		...
		if (!$('input[name="amount"]').val()) {
			...
			data.append('type', t);
			data.append('action', 'dicline');
			data.append('pre_date', cash.pre_date);
			data.append('acType', a);
			data.append('object', Object.keys($('input[name="object"]').data()).join(','));
			data.append('currency', cash.currency);
		//}
...
var store = {
    id: 0,
	pageApprove: function(a){
		$.post('/store/page_approve', {id: a}, function(){
			Page.get('/store/pages');
		});
	},
	blogApprove: function(a){
		$.post('/store/blog_approve', {id: a}, function(){
			Page.get('/store/blog');
		});
	},
	serviceApprove: function(a){
		$.post('/store/service_approve', {id: a}, function(){
			Page.get('/store/services');
		});
	},
	serviceDecline: function(id, type) {
		mdl.open({
			id: 'serviceDecline',
			title: 'Declining',
			content: 
			'<div class="uForm">' +
			'<div class="iGroup fw">' +
			'<textarea id="service_decline_comment" placeholder="Your comment.." />' +
			'</div>' +
			'<div class="sGroup"><button class="btn btnSubmit" onclick="store.doDeclineService(' + id + ', \'' + type + '\');">Save</button></div>' +
			'</div>',
		});
	},
	doDeclineService: function(id, type) {
		$.post('/store/service_decline', {
			id: id,
			comment: $('#service_decline_comment').val(),
			type: type
		}, function() {
			Page.get('/store/' + type);
		});
		mdl.close('serviceDecline');
	},
	...
	sendBlogCategory: function(f, event, id) {
        ...
        if (!form.find('input[name="name"]').val().length) {
            ...
            data.append('keywords', form.find('input[name="keywords"]').val());
	...
	sendNavPriority: function(e) {
        var data = {};
        $('.sUser').each(function(i, v) {
            data[i] = {
                id: $(v).attr('id').replace('nav_', '')
            }
        });
        if (data) {
            $.post('/store/stNavPriority', data, function(r) {
                //console.log(r);
            });
        }
    },
	...
	sendService: function(f, event, id) {
		...
		data.append('currency', $('select[name="currency"]').val());
		data.append('category', $('input[name="category"]').data() ? Object.keys($('input[name="category"]').data())[0] : 0);
		data.append('blog', $('input[name="blog"]').data() ? Object.keys($('input[name="blog"]').data())[0] : 0);
        ...
        if (err) {
			...
			if(pagePathname && pagePathname != $('input[name="pathname"]').val() && confirm('Create a redirect from an old link to a new one?'))
				data.append('old_pathname', pagePathname);
			$.ajax({
                type: 'POST',
				processData: false,
				contentType: false,
				url: '/store/send_service',
				data: data
			}).done(function(r) {
                loadBtn.stop($(event.target).find('button[type="submit"]'));
                if (r <= 0) {
    ...
    sendPost: function(f, event, id) {
        ...
        if (err) {
			...
			$.ajax({
                ...
                } else if (r.err == 'err_file_size') {
					alr.show({
                        class: 'alrDanger',
                        content: 'Uploaded photos are too large',
                        delay: 2
                    });
				} else if (r.err == 'err_image_type') {
					alr.show({
                        class: 'alrDanger',
                        content: 'Uploaded photos are have wrong type',
                        delay: 2
                    });
				} else if (r.err == 'no_access') {
					alr.show({
                        class: 'alrDanger',
                        content: 'You hav no access to do that',
                        delay: 2
...
/*             $.post('/store/send_post', data, function(r) {
	...
	moveToService: function(a) {
		if(confirm('Are you sure you want to convert the page into service?')){
			$.post('/store/move_to_service', {
				id : a
			}, function(b){
				if(parseInt(b)){
					Page.get('/store/services/edit/'+b);
				}
			});
		}
	},
    sendPage: function(f, event, id) {
        var btn = f.querySelector('button:focus'),
			submit = btn.value,
			data = new FormData(),
            form = $(f),
            err = null;
		...
		if(submit == 'preview' && !err){
			f.target = '_blank';
			f.action = 'https://yoursite.com/page/print';
			loadBtn.stop($(btn));
			return false;
		}
		event.preventDefault();
        if (err) {
			...
			if(pagePathname && pagePathname != data.get('pathname') && confirm('Create a redirect from an old link to a new one?'))
				data.append('old_pathname', pagePathname);
            ...
            if (r) {
                ...
                mdl.open({
                    ...
                    content: '<form  method="post" onsubmit="store.sendCategory(this, event, ' + (id || 0) + ');">\
						...
						<div class="iGroup">\
							<label>Description</label>\
							<textarea name="description">' + (cur.description || '') + '</textarea>\
						</div>\
						<div class="iGroup">\
							<label>Keywords</label>\
							<input type="text" name="keywords" value="' + (cur.keywords || '') + '">\
						</div>\
						<div class="iGroup">\
							<input type="hidden" name="parent">\
...
var total = {
    ...
    fTotal: function(){
		...
		//var profit = (parseFloat($('#doit').text().replace('$', ''))-income).toFixed(2);
/* 		$('#income').html('<b>'+profit+'</b>');
		if(profit > 0)
			$('.profit').css({'background':'#8bc34a','color':'#fff'});
		else
			$('.profit').css({'background':'#CE1212','color':'#fff'}); */
		...
		var tp = {inventory: [0,0],service: [0,0],purchase: [0,0]};
		$('#tbl_services > .tr').each(function(){
			var _e = $(this), _t = _e.find('.td').eq(3).text().trim(),
				_p = parseFloat(_e.find('.td').eq(2).find('span[data-price]').attr('data-price')),
				_p2 = parseFloat(_e.find('.td').eq(2).find('span[data-cost-price]').attr('data-cost-price')),
				_n = _t.toLowerCase();
			if(_n){
				if(!tp[_n])
					tp[_n] = [0,0];
		...
		var _parts = tp.inventory[1]+tp.purchase[1],
			_profit = (tp.inventory[0]+tp.purchase[0])-_parts,
			nincome = ((income2-_parts)+tp.service[0]+_profit).toFixed(2);
			
		$('#income').html('<b>'+nincome+'</b>');
		if(nincome > 0)
			$('.profit').css({'background':'#8bc34a','color':'#fff'});
		else
			$('.profit').css({'background':'#CE1212','color':'#fff'});
		
		$('#in-parts').text(_parts.toFixed(2));
		$('#in-service').text(tp.service[0].toFixed(2));
		$('#in-profit').text(_profit.toFixed(2));
		
		console.log(nincome, income2, _parts, _profit, tp.service[0]);
    }, 
...
var timer = {
	...
	confirm:function(e) {
		if(parseInt(e)){
			mdl.open({
				...
				content: $('<div/>', {
					class: 'aCenter',
					html: $('<div/>', {
						html: 'Are you sure you want to confirm?'
					})
				}).append($('<div/>', {
					class: 'cashGroup',
					html: $('<button/>', {
						class: 'btn btnSubmit ac',
						onclick: 'timer.accept2('+e+');',
						html: lang[159]
					})
				}).append($('<button/>', {
					class: 'btn btnSubmit dc',
		...
		mdl.close();
		$('#timers_list > div[data-id='+a+']').removeClass('stop').find('button.confirm').remove();
		$('.userList.tbl div[data-id='+a+']').removeClass('stop').find('button.confirm').remove();
		$.post('/timer/confirm', {id: a});
	}, timeStop: function(e) {
		...
		$.post('/analytics/feedback_report', {
			...
			if (r) {
				...
				makeStat.init({
					id: 'feedbacks_plot',
					s: 60, 
					w: $('#page').width() > 1472 ? $('#page').width() : 1472,
					h: 450, 
				...
				if(r.staffs){
					let _s = '';
					for(let k in r.staffs){
						_s += '<tr>\
							<td data-label="Staff:" style="line-height: 50px;">\
								<a href="/users/view/'+r.staffs[k].id+'" target="_blank">\
									'+(
										r.staffs[k].image ? '<img src="/uploads/images/users/'+r.staffs[k].id+'/thumb_'+r.staffs[k].image+'" class="miniRound">' : '<span class="fa fa-user-secret miniRound"></span>'
									)+'\
									'+r.staffs[k].name+' '+r.staffs[k].lastname+'\
								</a>\
							</td>\
							<td data-label="Amount of feedback:">'+r.staffs[k].count+'</td>\
							<td data-label="Details:">\
								<a href="/feedbacks/report_details?send_staff='+r.staffs[k].id+'&date_start='+r.date_start+'&date_finish='+r.date_finish+'">View details</a>\
							</td>\
						</tr>';
					}
					$('#sreport').html(_s);
...
function $_GET(a) {
	let r =  window.location.search.match(
		new RegExp("(\\?|&)" + a + "(\\[\\])?=([^&]*)")
	);
	return r ? r[3] : '';
}
...
var tasks = {
	tags: function(a){
		var textarea = a.target, val = textarea.value, data = val.match(/((Purchase|RMA|Issue|Invoice|Stock|Camera)\s#|@)(\w+)$/gim);
		if(data && data[0]){
			var exp = data[0].split('#');
			$.getJSON((exp[1] ? '/tags/'+exp[0].trim().toLocaleLowerCase()+'/'+exp[1] : '/users/all?query='+data[0].slice(1).replace('_',' ')), function(r){
				var ul = $('<ul>', {
					class: 'tags'
				});
				$(textarea).parent().find('ul').remove();
				if(!exp[1]){
					for(var k in r.list){
						$('<li>'+r.list[k].name+'</li>').appendTo(ul).click(function(){
							textarea.value = val.replace(data[0],'@id'+r.list[k].id+' ('+r.list[k].name+')');
							$(textarea).parent().find('ul').remove();
						});
					}	
				} else {
					if(r && r.status == 'OK'){
						var h = '';
						if(r.images)
							r.photo = '/uploads/images/inventory/'+r.id+'/'+r.images.split('|')[0];
						if(r.photo)
							h += '<img src="'+r.photo+'">';
						if(r.event)
							h += '<h4>Event: '+r.event+'</h4>';
						if(r.title)
							h += '<h4>'+r.title+'</h4>';
						if(r.descr)
							h += '<p>'+r.descr+'</p>';
						if(r.price)
							h += '<p class="ttPrice">Price: $'+r.price.replace('$', '')+'</p>';
						if(r.total)
							h += '<p class="ttPrice">Total: $'+r.total.replace('$', '')+'</p>';
						if(r.date)
							h += '<p class="ttPrice">Date: '+r.date+'</p>';
						$('<li>'+h+'</li>').appendTo(ul).click(function(){
							$(textarea).parent().find('ul').remove();
						});
					}
					console.log('not user', r);
				}
				$(textarea).after(ul);
			});
		}
            ...
            $.post('/users/objects', {}, function(or) {
				...
				mdl.open({
					...
					content: $('<div/>', {
							...
							html: '<option value="0">Right now</option><option value="1">Store opened</option><option value="2">Store closed</option><option value="3">Custom date</option>'
						}))).append($('<div/>', {
