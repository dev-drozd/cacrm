...
switch($route[1]){
	...
	*/
	case 'del_transfer':
		...
		$id = intval($_POST['id']);
		$sql = db_multi_query('SELECT from_manager, del FROM `'.DB_PREFIX.'_inventory_transfer` WHERE id = '.$id);
		if (($sql['from_manager'] == $user['id'] OR in_to_array(1, $user['group_ids'])) AND $sql['del'] == 0) {
			...
			db_query('DELETE FROM `'.DB_PREFIX.'_notifications` WHERE type = \'confirm_transfer\' AND id = '.$id);
	...
	*/
	case 'confirm_transfer':
		is_ajax() or die('Hacking attempt!');
		$id = intval($_POST['id']);
		$sql = db_multi_query('SELECT to_manager, to_store, received, inventory_ids, request FROM `'.DB_PREFIX.'_inventory_transfer` WHERE id = '.$id);
		if (($sql['to_manager'] == $user['id'] OR in_to_array(1, $user['group_ids'])) AND $sql['received'] == 0 AND $user['confirm_inventory_transfer'] AND $sql['request'] == 0) {
			...
			db_query('UPDATE `'.DB_PREFIX.'_inventory` SET object_owner = '.$sql['to_store'].', object_id = '.$sql['to_store'].' WHERE id IN ('.$sql['inventory_ids'].')');
			db_query('DELETE FROM `'.DB_PREFIX.'_notifications` WHERE type = \'confirm_transfer\' AND id = '.$id);
			die('OK');
		} else 
			die('no_acc');
	...
	*/
	case 'confirm_transfer_request':
		is_ajax() or die('Hacking attempt!');
		$id = intval($_POST['id']);
		$sql = db_multi_query('SELECT request FROM `'.DB_PREFIX.'_inventory_transfer` WHERE id = '.$id);
	...
	*/
	case 'make_transfer':
		...
		$trItems = ids_filter($_POST['trItems']);
		...
		if (!$request) {
			send_push($receive_staff, [
				...
				'message' => 'Transfer #'.$id.' added. Please, confirm'
			]);
		}
		die('OK');
	break;
	...
	*/
	case 'transfer':
		if ($route[2] == 'add' OR $route[2] == 'view' OR $route[2] == 'request') {
			$id = intval($route[3]);
			$meta['title'] = 'Transfer inventories';
			$row = [];
			$inventory = '';
			$forms = '';
			...
			if ($route[2] == 'view') {
				...
				', true) as $item) {
					$inventory .= '<div class="invItem">
						...
						($item['model'] ?: '')
					).'</a><span class="trPrice">$'.$item['price'].'</span></div>';
				}
			...
			$meta['title'] = 'Inventory transfers';
			$query = text_filter($_POST['query'], 255, false);
			$page = intval($_POST['page']);
			$count = 20;
			...
			if($sql = db_multi_query('
				...
				tpl_set('noContent', [
					'text' => 'There are no inventory transfers'
				], false, 'services');
			}
			$left_count = intval(($res_count-($page*$count)-$i));
			if($_POST){
	...
	*/
	case 'upcharge':
		...
		tpl_set('inventory/upcharge/main', [
			'res_count' => $res_count,
			'more' => $left_count ? '' : ' hdn',
			'upcharges' => $tpl_content['upcharge'] ?: '<div class="noContent">There are no upcharge services</div>'
		], [
			'add-service' => $user['add_service'],
			'create' => $user['create_inventory_transfer'],
	...
	*/
	case 'onsite':
		if ($route[2] == 'add' OR $route[2] == 'edit') {
			$id = intval($route[3]);
			$meta['title'] = ($route[2] == 'add' ? 'Add' : 'Edit').' on site service';
			$sql = [];
			...
			if ($user['edit_service']) {
				...
				tpl_set('inventory/onsite/form', [
					'title' => ($route[2] == 'add' ? 'Add' : 'Edit').' on site service',
					'id' => $id,
					'name' => $sql['name'],
			...
			if($sql = db_multi_query('
				...
				tpl_set('noContent', [
					'text' => 'There are no on site services'
				], false, 'services');
			}
			$left_count = intval(($res_count-($page*$count)-$i));
			if($_POST){
	...
	*/
	case 'to_store':
		is_ajax() or die('Hacking attempt!');
		db_query('UPDATE `'.DB_PREFIX.'_inventory` SET commerce = 1 WHERE id = '.intval($_POST['id']));
		die('OK');
	break;
	...
	*/
	case 'all':
		...
		$by_user = intval($_POST['by_user']);
		$store = intval($_POST['store']);
		$nIds = ids_filter($_POST['nIds']);
		$query = text_filter($_POST['query'], 100, false);
		$type = text_filter($_POST['type'], 100, false);
		...
		$objects = db_multi_query('
			...
			WHERE 1'.(
				...
				$by_user ? ($user ? ' AND i.customer_id = '.$user : ' AND i.customer_id < 0 ') : ''
			).(
				$lId ? ' AND i.id < '.$lId : ''
			).(
	...
	*/
	case 'send_inventory':
		...
		if (text_filter($_POST['intype'], 15, false) == 'service' AND !in_array('1', explode(',', $user['group_ids'])) AND !in_array('2', explode(',', $user['group_ids'])))
			die('no_acc');
		
		if (!intval($_POST['customer']) AND floatval($_POST['price']) < min_price(floatval($_POST['purchase-price'])))
			die('min_price');
		...
		if($_POST['opts']){
			...
			if($id AND text_filter($_POST['intype'], 15, false) == 'stock'){
				...
				if ($h['status'] != $status_id AND $h['sms'] == 1 AND $h['sms_form'] > 0) {
					...
					/* $usr = db_multi_query('SELECT u.id as uid, u.name, u.lastname, u.phone, f.content, s.sms as sms, s.sms_form as sms_form 
						FROM `'.DB_PREFIX.'_forms` f, 
							`'.DB_PREFIX.'_users` u
						WHERE f.id = '.$h['sms_form'].' 
							AND u.id = '.$h['customer']); */
			...
			if (text_filter($_POST['intype'], 20, false) == 'service') {
				...
				if (!$type AND $_POST['type_new']) {
					$tp = [];
					$tp = db_multi_query('SELECT name FROM `'.DB_PREFIX.'_inventory_types` WHERE name = \''.trim(text_filter($_POST['type_new'], 100, false)).'\'');
					if ($tp != []) die('tp_exists');
					$type = db_query('INSERT INTO `'.DB_PREFIX.'_inventory_types` SET name = \''.trim(text_filter($_POST['type_new'], 100, false)).'\'');
					$type = intval(
				...
				if (!$category AND $_POST['category_new']) {
					$cat = [];
					$cat = db_multi_query('SELECT name FROM `'.DB_PREFIX.'_inventory_categories` WHERE name = \''.trim(text_filter($_POST['category_new'], 100, false)).'\'');
					if ($cat != []) die('cat_exists');
					$category = db_query('INSERT INTO `'.DB_PREFIX.'_inventory_categories` SET name = \''.trim(text_filter($_POST['category_new'], 100, false)).'\'');
					$category = intval(
						mysqli_insert_id($db_link)
					);
				} */
			...
			db_query((
				$id ? 'UPDATE' : 'INSERT INTO'
					...
					'.($_POST['object_owner'] ? 'object_owner =\''.intval($_POST['object_owner']).'\',' : 'object_owner = '.intval($_POST['object']).',').'
	...
	*/
	case 'allStatuses':
		...
		$array = [];
		$curent = 0;
		$statuses = db_multi_query('SELECT SQL_CALC_FOUND_ROWS id, name, sort, not_priority, inventory, service FROM `'.DB_PREFIX.'_inventory_'.(intval($_POST['warranty']) ? 'warranty_' : '').'status` WHERE 1'.(
			$lId ? ' AND id < '.$lId : ''
		).(
			$query ? ' AND name LIKE \'%'.$query.'%\'': ''
	...
	*/
	case 'allTypes':
		...
		).($nIds ? ' AND t.id NOT IN('.$nIds.')' : '').' ORDER BY t.name ASC LIMIT 20', true);
		$res_count = intval(mysqli_fetch_array(db_query('SELECT FOUND_ROWS()'))[0]);
		die(json_encode([
			'list' => $types,
			'count' => $res_count,
		...
		if(($route[1] == 'add') OR (
			...
			if (($route[1] == 'add' OR ($route[1] == 'edit' AND $id)) AND $route[2] == 'service') {
				...
				tpl_set('inventory/service', [
					...
					'price' => $row['price'],
					'time' => $row['time'],
					'title' => ($route[1] == 'add' ? 'Add ' : 'Edit ').' Service',
					'type' => 'service',
					'send' => $id ? 'Save' : 'Send',
					'steps' => $steps,
				...
				tpl_set('inventory/'.(
					...
					'is_user' => (($route[1] == 'add' AND $_GET['user']) OR ($route[1] == 'step' AND $_GET['user']) OR $row['customer_id']),
					'add' => ($route[1] == 'add'),
					'edit' => ($route[1] == 'edit'),
					'charger' => $row['charger'] == 1,
					'inventory' => (
						$row['type'] == 'stock' OR isset($route[3]) OR (
		...
		if($id){
			...
			if ($row['type'] == 'service') {
				...
				tpl_set('inventory/view', [
					...
					'cr-date' => $row['cr_date'],
					'cr-price' => $row['cr_price'],
					'cr-user' => $row['cr_user'],
					'cr-name' => $row['cr_name'].' '.$row['cr_lastname'],
					'cn-date' => $row['cn_date'],
					'cn-price' => $row['cn_price'],
					'cn-user' => $row['cn_user'],
	...
	*/
	...
	case 'tradein':
	case 'deleted':
		$meta['title'] = ($route[1] == 'stock' ? 'Inventory' : ($route[1] == 'service' ? 'Services' : ($route[1] == 'tradein' ? 'Trade in confirmation' : ($route[1] == 'deleted' ? 'Deleted' : 'Stock&Services'))));
		$query = text_filter($_POST['query'], 255, false);
		$page = intval($_POST['page']);
		...
		$count = 20;
			...
			if($sql = db_multi_query('
				...
				WHERE i.customer_id = 0 '.(
					...
					$create ? 'AND i.create_id = '.$create.' ' : ''
				).(
					$confirm ? 'AND i.confirm_id = '.$confirm.' ' : ''
				).(
					$object ? 'AND i.object_id = '.$object.' ' : ''
				).($query ? '' : 'ORDER BY i.confirmed, p.confirmed, i.id DESC').' LIMIT '.($page*$count).', '.$count, true
			)){
				$i = 0;
					foreach($sql as $row){
						tpl_set($route[1] === 'tradein' ? 'inventory/item' : 'inventory/store_devices', [
							...
							'confirmed' => ($route[1] === 'tradein' ? ($row['trade_confirmed'] == 1 ? 'confirmed' : 'not-confirmed') : ($row['confirmed'] == 1 ? 'confirmed' : 'not-confirmed'))
						], [
							'service' => $row['type'] == 'service',
			...
			tpl_set('inventory/main', [
				...
				'title' => ($route[1] == 'stock' ? 'Inventory' : ($route[1] == 'service' ? 'Services' : ($route[1] == 'tradein' ? 'Trade in confirmation' : ($route[1] == 'deleted' ? 'Deleted' : 'Stock&Services')))),
				'type' => $route[1]
			], [
