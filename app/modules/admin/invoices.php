...
/* 		if($user['id'] == 16){
		...
		$service_charge = (int)$_POST['service_charge'];
		$refund_inventory = [
			'stock' => [],
			'tradein' => []
		];
		...
		if($service_charge > 0){
			$invoice['total'] += ($invoice['total']/100)*$service_charge;
		}
...
/* 		if($user['id'] == 16){
			echo 'UPDATE `'.DB_PREFIX.'_invoices` SET 
			pay_method = \''.$type.'\',
			transaction = 1,
			'.(
				$service_charge > 0 ? ' total = \''.floatval($invoice['total']).'\', service_charge = '.$service_charge.', ' : ''
			).'
					...
					if ($invoice['issue_id']) {
						db_query('
							INSERT INTO `'.DB_PREFIX.'_feedback_queue` SET
								customer_id = '.$invoice['customer_id'].', 
								store_id = '.intval($invoice['object_id']).', 
	...
	*/
	case 'objects':
		...
		$objects = db_multi_query('SELECT SQL_CALC_FOUND_ROWS id, name, tax, image, descr FROM `'.DB_PREFIX.'_objects` WHERE close = 0'.(
		...
		).' ORDER BY `id` DESC LIMIT 20', true, false, function($a){
			$a['image'] = '/uploads/images/stores/'.$a['id'].'/thumb_'.$a['image'];
			return [null, $a];
		});
...
<td width="440">'.$invoice_discount[0]['name'].'</td><td width="300">-'.$invoice_discount[0]['percent'].'%</td></tr>
			...
			$due = $total+$tax-$tradein - $row['paid'];
			//<br><img src="http://yoursite.com/uploads/logo.png" width="200" height="100" x="350" y="30" style="max-width: 300px">
...
/* 	case 'send': 
	...
	*/
	...
	case 'save_service_charge':
		is_ajax() or die('Hacking attempt!');
		$config['cash_charge'] = (int)$_POST['cash_charge'];
		$config['credit_charge'] = (int)$_POST['credit_charge'];
		conf_save();
		die;
	break;
	
	case 'service_charge':
		tpl_set('invoices/discount/scharge', [
			'credit-charge' => $config['credit_charge'],
			'cash-charge' => $config['cash_charge']
		], [], 'content');
	break;
	...
	default:
		...
		if($sql = db_multi_query('
			...
			$status === 'paid' ? 'AND i.conducted = 1 ' : (
				$status === 'unpaid' ? 'AND i.conducted = 0 ' : (
					$status === 'partial' ? 'AND i.conducted = 0 AND i.paid > 0 ' : (
						$status === 'transactions' ? 'AND i.paid > 0 ' : ''
					)
				)
			)
		).(
			...
			foreach($sql as $row){
				tpl_set('invoices/item', [
					...
					'paid' => number_format($row['paid'], 2, '.', ''),
					'pay' => $row['paid'] > 0 ? $row['pay_method'] : 'Not selected',
