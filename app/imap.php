...
class imap_driver
{
	...
	public function get_body_from_uid($uid, $part = 2)
    {
		
        $this->command("FETCH $uid BODY[".$part."]");
        if (preg_match('~^OK~', $this->last_endline)) {
            array_shift($this->last_response); // skip first line
            $body = '';
            $body = implode(' ', $this->last_response);
			$body = ($part != 'TEXT' ? trim(quoted_printable_decode($body)) : $body);

			if (strlen($body) > 1) {
				return $body;
			} elseif (strlen($body) <= 1 AND $part != 'TEXT') {
				$this->get_body_from_uid($uid, 'TEXT');
			}
        } else {
            $this->error = join(', ', $this->last_response);
            $this->close();
	...
	/* public function append($message, $folder)
    {
        $this->command("APPEND \"$folder\" (\Seen) {".strlen($message)."}");
        $this->command($message);
		print $this->last_endline;
        if (preg_match('~^OK~', $this->last_endline))
            return true;
        else
            return false;
